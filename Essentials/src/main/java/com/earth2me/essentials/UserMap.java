...
public class UserMap extends CacheLoader<String, User> implements IConf
{
        ...
        if (uuid != null)
        {
            ...
            if (name != null && name.length() > 0)
            {
                ...
                else if (!names.get(keyName).equals(uuid))
                {
                    if (replace)
                    {
                        ess.getLogger().info("Found new UUID for " + name + ". Replacing " + names.get(
                                keyName).toString() + " with " + uuid.toString());
                        names.put(keyName, uuid);
                        uuidMap.writeUUIDMap();
                    }
                    else
                    {
                        ...
                        {
                            ess.getLogger().info(
                                    "Found old UUID for " + name + " (" + uuid.toString() + "). Not adding to usermap.");
                        }
                    }
                }
            }
    ...
    @Override
    public User load(final String stringUUID)
            throws Exception
    {
